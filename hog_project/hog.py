...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    # we declare two variables here, one to keep track of the total sum of the dice
    # and another boolean value to check for the PIG OUT case
    sum = 0
    num_one_check = False
    
    # while the num_rolls is greater than 0
    while num_rolls > 0:
        # roll the dice and set it equal to current
        current = dice()
        # if the value = 1 then we set the boolean value true
        if current == 1:
            num_one_check = True
        else:
            # otherwise we add it to the variable sum
            sum += current
        # subtract 1 to num_rolls and do the while loop again
        num_rolls = num_rolls - 1
    # this is the test, if num_one_check has been set to true then return 1
    # otherwise return the sum total
    if num_one_check:
        return 1
    else:
        return sum
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        # implement the free bacon rule. Convert the opponents score to a string,
        # and check for max before turning it back into a int to + 1
        return 1 + int(max(str(opponent_score)))
    else:
        # roll the dice
...
def select_dice(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    # a simple math check, if your score + opponent score adds up and 
    # and is evenly divisible by 7 then select the four sided dice
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    # while your score and the opponent score is below 100, play the game!
    while score < goal and opponent_score < goal:
        # if who is == 0 then it is player 1's turn
        if who == 0:
            # use strategy 0 to select # of rolls
            num_rolls = strategy0(score, opponent_score)
            # total the number into the local variable score
            score += take_turn(num_rolls, opponent_score, select_dice(score, opponent_score))
        # check if who == 1, then it is player 2's turn
        elif who == 1:
            # use strategy 1 to select rolls
            num_rolls = strategy1(opponent_score, score)
            # add the total number of points into variable opponent_score
            opponent_score += take_turn(num_rolls, score, select_dice(opponent_score, score))
        # this is the hog wild special rule, we can use the or statement to check the score * 2 
        if score == opponent_score * 2 or opponent_score == score * 2:
            # simple swap function to swap score, and opponent score values
            score, opponent_score = opponent_score, score
        # run the other() function to determine who goes next
        who = other(who)
    # return your score and the opponents score
...
def make_averaged(fn, num_samples=1000):
    ...
    def helper_average(*args):
        # set two variables one for counting and one for the sum
        total = 0
        i = 0
        # while i is less than 1000
        while i < num_samples:
            # continuously add the arbitrary numer of arguments into total
            total += fn(*args)
            # increment i by 1 and repeat the loop
            i += 1
        # return the average
        return total / num_samples
    # return the function into make_averaged
    return helper_average
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    """
    "*** YOUR CODE HERE ***"
    # three variables one for the avg score, one for counting, one for the terminate
    avg_score = 0
    i = 1
    max_dice = 10
    ...
    while i <= max_dice:
        # set average_roll to the function call of make_averaged
        average_roll = make_averaged(roll_dice, 1000)
        # avg score will have the value of the average 
        avg_score = average_roll(i, dice)
        # print out the result and increment i by 1 and repeat
...
def bacon_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    # because we dont have a literal free_bacon function we can just
    # reimplement the code that would do the same. If that ends up being greater than or
    # equal to bacon margin then return 0 otherwise return the baseline
    if (1 + int(max(str(opponent_score)))) >= BACON_MARGIN:
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    # premptively calculate the bacon points 
    bacon_points = 1 + int(max(str(opponent_score)))
    # if bacon points + score raised to the power of 2, and score + bacon points is lower than the opponent score
    # then we can return 0
    if (score + bacon_points) * 2 == opponent_score and score + bacon_points < opponent_score:
        return 0
    # same with if the bacon_points is greater than or equal to BACON_MARGIN
    # we need this because sometimes while the score + bacon_points it may not result in
    # double of the score
    elif bacon_points >= BACON_MARGIN and score + bacon_points < opponent_score:
        return 0
    # otherwise return the BASELINE_NUM_ROLLS
    else:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    difference_in_scores = score - opponent_score
    num_rolls = 4
    
    if difference_in_scores > 20:
        num_rolls = 3
    elif difference_in_scores < -20:
        num_rolls = 5
    elif score >= 100:
        num_rolls = 0
